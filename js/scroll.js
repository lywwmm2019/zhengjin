// JavaScript Document
	...
	$('#center').bxCarousel({
		display_num: 4, 
		move: 1,
		prev_image: '../images/arrow_left.gif',
		next_image: '../images/arrow_right.gif',
		margin: 20 
	});
	$('#center_large').bxCarousel({
		display_num: 1, 
		move: 1,
		prev_image: 'none',
		next_image: 'none',
...
$this.parent('.bx_container').wrap('<div class="bx_wrap"></div>').css({'position':'relative','width':wrap_width,'overflow':'hidden','background':'none'}).before(controls);var w=li.slice(0,options.display_num).clone();var last_appended=(options.display_num+options.move)-1;$this.empty().append(w);get_p();get_a();$this.css({'position':'relative','left':-(seg)});$this.parent().siblings('.next').click(function(){slide_next();/*clearInterval(j);*/clicked=true;return false;});$this.parent().siblings('.prev').click(function(){slide_prev();/*clearInterval(j);*/clicked=true;return false;});if(options.auto){start_slide();if(options.auto_hover/*&&clicked!=true*/){$this.find('li').live('mouseenter',function(){/*if(!clicked){clearInterval(j);}*/clearInterval(j);});$this.find('li').live('mouseleave',function(){/*if(!clicked){start_slide();}*/start_slide();});}}
